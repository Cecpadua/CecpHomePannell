...
watch(()=>dialog.value, (value) => {
  if(!value){
    store.commit('setDialog', {dialog: 'CameraAdd', value: false})
  }
})
...
const create = () => {
  ...
  fetch(store.state.server_ip +  '/api/camera', {
    method: 'POST',
    headers: {
    ...
    socketService.send(JSON.stringify({ type: 'camera', action: 'add' }));
    }
    loading.value = false
    dialog.value = false
  }).catch(() => {
